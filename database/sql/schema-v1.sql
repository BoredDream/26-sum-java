...
CREATE TABLE process_document (
  ...
  stage_id BIGINT NULL COMMENT '关联阶段任务ID',
...
CREATE TABLE attendance_task (
  ...
  require_location TINYINT NOT NULL DEFAULT 0 COMMENT '是否启用定位签到：0-否，1-是',
  location_lng DECIMAL(10, 7) NULL COMMENT '签到位置经度',
  location_lat DECIMAL(10, 7) NULL COMMENT '签到位置纬度',
  location_radius INT NULL DEFAULT 500 COMMENT '签到允许半径（米），默认500米',
  location_name VARCHAR(255) NULL COMMENT '签到位置名称',
