...
public interface TeamMapper {
    @Select("SELECT ti.team_id AS id, ti.team_name, ti.leader_id, ti.team_intro AS introduction, " +
            "CASE WHEN ti.team_status = 3 THEN 'DISSOLVED' WHEN ti.team_status = 2 OR EXISTS (SELECT 1 FROM topic_selection ts WHERE ts.team_id = ti.team_id AND ts.audit_status = 1) THEN 'SELECTED' ELSE 'BUILDING' END AS status, " +
    ...
    @Select("SELECT ti.team_id AS id, ti.team_name, ti.leader_id, ti.team_intro AS introduction, " +
            ...
            "FROM team_info ti WHERE ti.is_deleted = 0 AND ti.team_status = 0 " +
            "AND NOT EXISTS (SELECT 1 FROM topic_selection ts WHERE ts.team_id = ti.team_id AND ts.audit_status = 1) " +
