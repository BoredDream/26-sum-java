...
public interface ProcessDocumentMapper {
    ...
    @Insert("INSERT INTO process_document(team_id, topic_id, document_name, document_type, project_stage, file_path, file_size, version_no, uploader_id, audit_status, upload_time, create_time) " +
            "VALUES(#{teamId}, #{topicId}, #{documentName}, #{documentType}, #{projectStage}, #{storedPath}, #{fileSize}, #{versionNo}, #{uploaderId}, CASE WHEN #{status} = 'RETURNED' THEN 2 WHEN #{status} = 'REVIEWED' THEN 1 ELSE 0 END, NOW(), NOW())")
    ...
    @Select("SELECT document_id AS id, team_id, topic_id, document_name, document_type, project_stage, version_no, " +
            ...
            "teacher_feedback, feedback_teacher_id, upload_time, update_time AS feedback_time " +
    ...
    @Select("SELECT d.document_id AS id, d.team_id, d.topic_id, d.document_name, d.document_type, d.project_stage, d.version_no, " +
            ...
            "d.teacher_feedback, d.feedback_teacher_id, d.upload_time, d.update_time AS feedback_time " +
